...
def create_subarray(counts_array, bdt_eff_array):
    ...
    mask = np.logical_and(bdt_eff_original>=bdt_eff_array[0], np.round(bdt_eff_original,2)<=bdt_eff_array[-1])
...
def unbinned_mass_fit(data, eff, bkg_model, output_dir, cent_class, pt_range, ct_range, split, bins=38):
    ...
    # fit data
    fit_function.fitTo(roo_data, ROOT.RooFit.Range(2.96, 3.04), ROOT.RooFit.Extended(ROOT.kTRUE))
    ...
    signal_counts = signal.createIntegral(ROOT.RooArgSet(mass), ROOT.RooArgSet(mass), 'signal region').getVal() * n_sig.getVal()
    background_counts = background.createIntegral(ROOT.RooArgSet(mass), ROOT.RooArgSet(mass), 'signal region').getVal() * n_bkg.getVal()
