...
public:
  ...
  void Fill(const SHyperTriton3O2 &RHyper, TF1 *blastWave, double max);
  // void SetName(const char *name);
  bool AcceptCandidateBW(float gPt, float centrality, TF1 *blastWave, double max);
...
TableO2::TableO2(bool isMC)
{
  ...
  if (isMC)
  {
    ...
    tree->Branch("rej_accept", &rej_accept);
  }
};
...
void TableO2::Fill(const RHyperTriton3O2 &RHyper)
{
  ...
  trigger = RHyper.trigger;

  tree->Fill();
}

void TableO2::Fill(const SHyperTriton3O2 &SHyper, TF1 *blastWave, double max)
{
  gPt = SHyper.gPt;
  gPhi = SHyper.gPhi;
  ...
  rej_accept = AcceptCandidateBW(gPt, centrality, blastWave, max);
...
bool TableO2::AcceptCandidateBW(float gPt, float centrality, TF1 *blastWave, double max)
{


  float bwNum = blastWave->Eval(gPt) / max;
